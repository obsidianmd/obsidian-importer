...
export interface NoteTemplatePreview {
	...
	path?: string;
	diagnostics?: string[];
	/** False for fatal errors, not warnings. */
	valid?: boolean;
}
...
export interface NoteTemplateConfiguratorOptions {
	...
	/** Importer-owned properties shown read-only. */
...
type NativeTemplateEditorFactory = typeof createNativeTemplateEditor;
...
async function createNativeTemplateEditor(
	...
	// MetadataEditor is internal, so mount a detached MarkdownView around a
	// synthetic file that is never read or written.
...
function renderProperties(container: HTMLElement, properties: Record<string, unknown>): void {
	...
	// Match MetadataEditor's DOM so Obsidian and themes style the read-only preview.
...
export class NoteTemplateConfigurator {
	...
	constructor(
		private readonly options: NoteTemplateConfiguratorOptions,
		private readonly createEditor: NativeTemplateEditorFactory = createNativeTemplateEditor,
	) {
	...
	async show(container: HTMLElement, buttonsEl: HTMLElement): Promise<NoteTemplateEditorConfig | null> {
		return await new Promise(resolve => {
			...
			let resetToDefault = async (): Promise<void> => {};
			...
			// Keep the current preview mounted to avoid a layout jump while rerendering.
			...
			const resetButton = new ButtonComponent(previewSetting.controlEl)
				.setButtonText(i18n.template.buttonResetTemplate())
				.onClick(() => void resetToDefault());
			const updateResetButton = (): void => {
				resetButton.buttonEl.toggle(!this.path && this.template !== this.options.defaultTemplate);
			};
			updateResetButton();
			...
			const startEditing = async (): Promise<void> => {
				...
				try {
					const candidate = await this.createEditor(
						this.options.app,
			...
			const loadTemplate = async (): Promise<void> => {
				...
				// Ignore incomplete paths while the user types.
