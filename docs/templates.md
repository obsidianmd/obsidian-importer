...
| Google Keep | `isArchived`, `isPinned`, `isTrashed`, `color`, `labels`, `sharees`, and `annotations`. The source's `title` remains available as `source.title`; labels are exposed as a simple array for list properties. The converted note is available through the shared `content` and `body` variables. |
...
| Apple Notes | `originalTitle`, containing the title stored by Apple Notes before note-title templating or filename sanitization, and `isPinned`, indicating whether the note was pinned. Shared `ctime`, `mtime`, and `sourceId` provide its creation time, modification time, and identifier. |
